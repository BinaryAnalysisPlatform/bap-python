...
def _parse_str(in_c, in_s, i, objs, stk):
    ...
    if sys.version_info > (3,):
        ...
        parent = objs[k] = (
            (in_s[i + 1 : endpos]).encode("utf-8").decode("unicode_escape")
        )
    else:
...
def _parse_end(in_c, in_s, i, objs, stk):
    ...
    if in_c == ",":  # add blank object and move on
        ...
        if ptyp == "[":
            ...
            parent = objs[j] = getattr(bir, name)(
                *parent.get("children", ())
            )  # pylint: disable=redefined-variable-type
...
EVALFREE_ADT_PARSER = {"format": "adt", "load": parser}
