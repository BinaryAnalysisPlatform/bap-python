...
class BapError(Exception):
    ...
    def info(self):
        ...
        Invoked as: {2}
        """.format(
            self.out, self.err, " ".join(self.cmd)
        )
...
class MalformedOutput(BapError):
    ...
    def __str__(self):
        return "\n".join(
            ["expected a valid Python expression, but got", str(self.exn), self.info()]
        )
...
class Failed(BapError):
    ...
    def __str__(self):
        return "\n".join(["exited with return code {0}".format(self.code), self.info()])
...
class Killed(BapError):
    ...
    def __str__(self):
        return "\n".join(["received signal {0}".format(self.signal), self.info()])


adt_project_parser = {"format": "adt", "load": bir.loads}
