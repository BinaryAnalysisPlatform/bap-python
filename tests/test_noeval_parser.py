...
integer_types = (int, long) if sys.version_info < (3,) else (int,) # pylint: disable=invalid-name
...
def conv(s, i, mayint=True): # pylint: disable=invalid-name
    ...
    elif mayint and s[i:i+2] == '0x': # try to normalize integers in hex representation
        j = i + 2
...
def _compare_proj_str(estr, possible_actual_strs):
    ...
    '''
    exceptions = []
    for aidx, astr in enumerate(possible_actual_strs): # so we can try both ADT_repr implementations
        try:
            i = 0
            j = 0
            a_len = len(astr)
                ...
                if achar == echar:
                    ...
                    if estr[j] == '\\': # try the simple version of achar
                        ...
                        if achar_new == echar:
                            ...
                            j = j_new
                            continue
                    if isinstance(achar, integer_types) and not isinstance(echar, integer_types):
                        ...
                        try:
                            eint = int(estr[j:k])
                            info = 'int mismatch at i=%d j=%d %d!=%d' % (i, j,
                                                                         achar,
                                                                         eint)
            ...
            if (aidx+1) == len(possible_actual_strs): # then we're on last one so raise all
...
if HAVE_PYTEST:
    ...
    test_compare_to_old_verybig = pytest.mark.slow(test_compare_to_old_verybig) # pylint: disable=invalid-name
